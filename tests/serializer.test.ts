...
test('Serialize Unsupported Types No-Fallback (Invalid)', (done) => {
	expect.assertions(2);
	try {
		serialize(() => {});
		done.fail('Serialize should fail.');
	} catch (error) {
...
test('Serialize Unsupported Object Types (Invalid)', (done) => {
	expect.assertions(2);
	try {
...
test('Serialize Unsupported Serialized Types (Invalid)', (done) => {
	expect.assertions(2);
	try {
		serialize(
			() => {},
...
test('Serialize String with Null Pointer (Invalid)', (done) => {
	expect.assertions(2);
	try {
